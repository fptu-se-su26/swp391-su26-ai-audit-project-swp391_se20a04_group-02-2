...
const ALERT_TYPE_LABEL = {
  extreme_heat: "Nắng nóng", extreme_cold: "Rét đậm",
  heavy_rain: "Mưa lớn", strong_wind: "Gió mạnh", drought: "Hạn hán",
};

function getHeroGradient(icon) {
  if (!icon) return "linear-gradient(145deg, #1e3a5f 0%, #1e40af 55%, #2563eb 100%)";
  ...
  return "linear-gradient(145deg, #1e3a5f 0%, #1e40af 55%, #2563eb 100%)";
}
  ...
  return (
    <div className="ins-page">
        ...
        <div className="ins-risk-content">
          ...
          <p className="ins-risk-desc">
            {activeAlertTypes.length > 0
              ? <>Đang có cảnh báo: <strong>{activeAlertTypes.map(t => ALERT_TYPE_LABEL[t] || t).join(", ")}</strong>. Doanh nghiệp cần cân nhắc các gói bảo hiểm nông sản phù hợp.</>
              : "Thời tiết ổn định. Đây là thời điểm tốt để đánh giá rủi ro và cân nhắc bảo hiểm nông nghiệp cho các hợp đồng thu mua."
            }
          </p>
          {weather && (
      ...
      <div className="ins-section-header">
        <h3 className="ins-section-title">Các gói bảo hiểm nông nghiệp tham khảo</h3>
        <p className="ins-section-sub">Thông tin các chương trình bảo hiểm phổ biến — hỗ trợ doanh nghiệp tư vấn đối tác nông dân</p>
      </div>
            ...
            <div key={prog.id} className={`ins-program-card ${isOpen ? "open" : ""}`} style={{ "--accent": prog.accentColor }}>
              ...
              >
                ...
                <div className="ins-program-actions">
                  <a href={`tel:${prog.hotline.replace(/\s/g, "")}`} className="ins-hotline-btn" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="ins-coverages-grid">
                    <div className="ins-coverages-col">
                      ...
                      <ul className="ins-coverage-list">
                        {prog.coverages.map((c, i) => <li key={i}>{c}</li>)}
                      </ul>
                    </div>
                    <div className="ins-coverages-col">
                      <div className="ins-col-label"><FiInfo size={13} /> Phù hợp với</div>
                      <ul className="ins-coverage-list">
                        {prog.suitable.map((s, i) => <li key={i}>{s}</li>)}
                      </ul>
                    </div>
                  </div>
      ...
      <div className="ins-govt-card">
        ...
        <div className="ins-govt-body">
          ...
          <p className="ins-govt-desc">
            ...
            Nông dân không thuộc diện nghèo được hỗ trợ <strong>20%</strong>. Doanh nghiệp có thể hướng dẫn đối tác nông dân liên hệ UBND xã/phường để đăng ký.
          </p>
        </div>
      </div>
    </div>
...
export default function WeatherInsuranceContent() {
  ...
  useEffect(() => { loadData(selectedProvince); }, [selectedProvince, loadData]);
  ...
  return (
    <>
      <div className="breadcrumb">
        ...
        <span>Thời tiết &amp; Bảo hiểm</span>
      </div>

      <div className="wi-page-header">
        <div>
          <h2>Thời tiết &amp; Bảo hiểm</h2>
          <p className="wi-page-subtitle">Theo dõi thời tiết và đánh giá rủi ro cho các hợp đồng thu mua nông sản.</p>
        </div>
      </div>
        ...
        <>
          ...
          {activeSection === "thresholds" && (
            <div className="weather-thresholds-card">
              ...
              <p className="wt-desc">Hệ thống tự động phát cảnh báo khi các chỉ số vượt ngưỡng dưới đây. Kiểm tra mỗi 6 giờ.</p>
          ...
          {activeSection === "insurance" && (
            <InsuranceSection weather={weather} alerts={alerts} />
          )}
        </>
      )}
    </>
